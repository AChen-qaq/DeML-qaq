...
def main():
    ...
    for dataset_name, dataset_config_name in test_task_list:
        ...
        if args.debug:
            ...
            raw_datasets = raw_datasets.select(range(min(200, len(raw_datasets))))
        ...
        for template_id in template_list:
            ...
            if args.dataset_type == 'ga':
                ...
                # for label_type in label_type_set:
                #     single_label_dataset = filtered_dataset.filter(lambda x: x[label_key] == label_type)
                #     single_label_dataset = single_label_dataset.shuffle(seeds=42)
                #     dataset_per_type.append(single_label_dataset)
                # ga_dataset_list = []
                # example_num_per_label = math.ceil(32 / len(label_type_set))
                # for dataset in dataset_per_type:
                #     ga_dataset_list.append(dataset.select(range(min(example_num_per_label, len(dataset)))))
                #
                # filtered_dataset = concatenate_datasets(ga_dataset_list)
